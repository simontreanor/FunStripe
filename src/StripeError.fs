...
module StripeError =
    ...
    type ErrorObject = {
        ...
        PaymentMethodType: PaymentMethodType option
    ...
    with
        static member New(?charge: string, ?code: string, ?declineCode: string, ?docUrl: string, ?message: string, ?metadata: Map<string, string>, ?param: string, ?paymentIntent: PaymentIntent, ?paymentMethod: PaymentMethod, ?paymentMethodType: PaymentMethodType, ?setupIntent: SetupIntent, ?source: Source, ?``type``: ErrorType) =
            {
                Charge = charge
                Code = code
