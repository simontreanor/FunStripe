...
module StripeIdsBuilder =
    ...
    let private emitSource (version: string) (markers: string list) : string =
        ...
        appendLine ""
        appendLine "    /// A generic paginated list of Stripe objects, including pagination metadata."
        appendLine "    /// Defined here (independent of any per-domain modular file) so all consumers"
        appendLine "    /// of the modular Stripe.{Domain} namespaces can reference the same wrapper."
        appendLine "    type StripeList<'T> = {"
        appendLine "        ///The list of objects in this page."
        appendLine "        Data: 'T list"
        appendLine "        ///True if this list has another page of items after this one that can be fetched."
        appendLine "        HasMore: bool"
        appendLine "        ///The URL where this list can be accessed."
        appendLine "        Url: string"
        appendLine "    }"
        appendLine "        with"
        appendLine "            ///String representing the object's type. Always has the value `list`."
        appendLine "            member _.Object = \"list\""
